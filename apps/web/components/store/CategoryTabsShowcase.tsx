...
type CategoryTabsShowcaseProps = {
  title?: string;
  titleBn?: string;
...
function ProductGrid({
    ...
    return (
      <div
        className={cn(
          ...
          cols === 5 && 'grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5',
          cols === 6 && 'grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6'
        )}
      >
  ...
  if (style === 'carousel') {
    return (
      <div ref={carouselRef} className="scrollbar-hide overflow-x-auto scroll-smooth">
        <div
          className="flex gap-3 sm:gap-4"
          style={{ scrollSnapType: 'x mandatory', minWidth: 'max-content' }}
        >
...
function AllProductsTab({
  ...
  const { data, isLoading } = useProducts({ sort: 'rating', limit: cols * (grid?.rows ?? 2) });
...
export function CategoryTabsShowcase({
  title,
  titleBn,
  titleAlign = 'left',
  tabs,
  ...
  // Build the display list: optional "All" tab + configured tabs
  type DisplayTab = {
    id: string;
    label: string;
    ...
    <section className="mx-auto max-w-screen-2xl px-2 py-8 sm:px-3 sm:py-10 lg:px-4 lg:py-14">
      ...
      <div className="border-border mb-6 border-b pb-3">
        ...
        {isCenter ? (
          ...
          /* Left/Right layout — single-line scrollable tabs on mobile, no overlap */
